...
insert into Emp (Id,Fname)values (2,'Remy')
select * from Emp
...
--Violation of PRIMARY KEY constraint 'PK__Emp__3214EC07552F80DA'. Cannot insert duplicate key in object 'dbo.Emp'. The duplicate key value is (2).
...
insert into Emp values (2,'Rina','Kumari','Deldi')
select * from Emp
...
--The INSERT statement conflicted with the CHECK constraint "CK__Emp__Salary__4222D4EF". 
--The conflict occurred in database "OurDb", table "dbo.Emp", column 'Salary'.
insert into Emp values (2,'Rina','Kumari','Delhi',69000)
--The INSERT statement conflicted with the CHECK constraint "CK__Emp__Salary__4222D4EF". The conflict occurred in database "OurDb", table "dbo.Emp", column 'Salary'.
...
select * from Emp
insert into Emp values (2,'Riya','987654321a')
--The INSERT statement conflicted with the CHECK constraint "CK__Emp__Mobile__44FF419A". The conflict occurred in database "OurDb", table "dbo.Emp", column 'Mobile'.
...
--Violation of UNIQUE KEY constraint 'UQ__Emp__6FAE078263E568A9'. 
...
BasicSalary float,
HRA as BasicSalary*0.10persisted,
...
--The INSERT statement conflicted with the FOREIGN KEY constraint "FK__Product__product__34E8D562". 
--The conflict occurred in database "master", table "dbo.Category", column 'CatId'.
...
p.ProductCategory'Category Product',c.CategoryName'Category Name'
	from Product p join Category c
	on p.ProductCategory=c.CatId

drop table Product join Category 
on p.ProductCategory=c.CatId
